...
export const deletePCandCP = createAsyncThunk(
  "/deletePCandCP",
  async (url, thunkAPI) => {
    try {
...
const deletePCandCPSlice = createSlice({
  name: "deletePCandCP",
  initialState,
  reducers: {
  ...
  extraReducers: {
    [deletePCandCP.fulfilled]: (state, { payload }) => {
      state.users = [];
    ...
    [deletePCandCP.rejected]: (state, { payload }) => {
